...
import { type ExperienceEntry, experience } from '@/data/portfolio';
...
function hasClient(e: ExperienceEntry): e is ExperienceEntry & { client: string } {
  return 'client' in e && typeof (e as { client?: unknown }).client === 'string';
}

function hasCurrent(e: ExperienceEntry): e is ExperienceEntry & { current: true } {
  return 'current' in e && (e as { current?: unknown }).current === true;
}

function hasProgression(e: ExperienceEntry): e is ExperienceEntry & { progression: string[] } {
  return 'progression' in e && Array.isArray((e as { progression?: unknown }).progression);
}

function hasAchievement(e: ExperienceEntry): e is ExperienceEntry & { achievement: { title: string; detail: string } } {
  return 'achievement' in e && typeof (e as { achievement?: unknown }).achievement === 'object';
}
    ...
    <Section id="experience" className="border-t border-default">
      ...
      >
        {experience.map((job, i) => {
          ...
          // Use first letters of each word for the monogram, not slice(0,2)
          ...
          return (
            ...
            >
              {/* Accordion header — key highlights always visible, no click required */}
              <button
                onClick={() => setOpenIdx(open ? -1 : i)}
                aria-expanded={open}
                className="flex w-full items-start justify-between gap-4 px-5 py-4 text-left sm:px-6 sm:py-5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-accent-500"
              >
                <div className="flex items-start gap-4">
                  <div className={`mt-0.5 flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border bg-[rgb(var(--bg))] font-mono text-sm font-semibold text-accent-500 transition-[border-color] duration-200 ${
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-baseline gap-x-3 gap-y-0.5">
                      ...
                      {hasCurrent(job) && (
                        <span className="rounded-full bg-accent-500/15 px-2 py-0.5 font-mono text-[10px] font-medium text-accent-500">
                          Current
                        </span>
                      )}
                    </div>
                    ...
                    {/* Key highlights — scannable without opening the accordion */}
                    <ul className="mt-2.5 space-y-1" aria-label="Key highlights">
                      {job.keyHighlights.map((kh) => (
                        <li key={kh} className="flex items-start gap-2 text-xs text-muted">
                          <span className="mt-1.5 h-1 w-1 shrink-0 rounded-full bg-accent-500/50" aria-hidden="true" />
                          {kh}
                        </li>
                      ))}
                    </ul>
                ...
                <ChevronDown
                  className={`mt-1 h-4 w-4 shrink-0 text-muted transition-transform duration-200 ${open ? 'rotate-180' : ''}`}
                  aria-hidden="true"
                />
              </button>

              {/* Accordion body — full detail on expand */}
                ...
                {open && (
                  ...
                  >
                    <div className="border-t border-default px-5 pb-6 pt-5 sm:px-6">
                      ...
                      {/* Highlights */}
                      <ul className="mt-5 space-y-2" aria-label="Detailed responsibilities">
